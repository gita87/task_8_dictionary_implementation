...
from app.qa import _parse_csv, _image_metrics
...
def test_legacy_and_common_progress_and_cancellation():
    legacy, shared = [], []
    convert_dictionary_docx(make_docx(False), progress_callback=legacy.append,
                            common_progress_callback=shared.append)
    assert [e.stage for e in shared] == [
        'reading', 'validating', 'parsing', 'parsing', 'writing', 'completed']
    ...
    with pytest.raises(ConversionError) as caught:
        convert_dictionary_docx(make_docx(False), cancellation=token,
                                common_progress_callback=cancel_at_write)
...
def test_qa_reader_restores_global_csv_limit():
    content = rows_to_csv(DICTIONARY_COLUMNS, [dict(
        unique_id='0001', word='a', definition='x' * 150_000, image='NA')])
